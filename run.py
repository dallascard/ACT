...
from models.baseline_model import TextBaseline
from models.act_model import AttentionCnnAct
...
def main():
    ...
    parser.add_argument('--model', type=str, default='baseline', metavar='N',
                        help='Model to use [baseline|act]')
    ...
    parser.add_argument('--subset', type=str, default=None, metavar='N',
                        help='Subset for amazon datasets')
    ...
    # Model Options
    parser.add_argument('--glove-file', type=str, default='data/glove/glove.6B.300d.txt.gz', metavar='N',
                        help='Glove vectors')
    ...
    if args.model == 'baseline':
        ...
        model = AttentionCnnAct(args, vocab, embeddings_matrix)
    else:
...
def test(args, model, test_loader, ref_loader, name='Test', return_acc=False):
    ...
    for batch_idx, (data, target, indices) in enumerate(test_loader):
        ...
        if args.model != 'ordinal':
            ...
            labels = torch.zeros([batch_size, args.n_classes-1]).byte()
            for i in range(args.n_classes-1):
                labels[:, i] = target > i
            if args.cuda:
                labels = labels.cuda()
            pred = output['probs'].exp() > 0.5

            correct_items = pred.eq(labels).min(1, keepdim=True)[0]
            correct += correct_items.sum().item()

        all_indices.extend(list(to_numpy(indices, args.cuda)))
        if not return_acc:
            true_labels.extend(list(to_numpy(target, args.cuda)))
            pred_probs.append(to_numpy(output['probs'].exp(), args.cuda))
            #all_indices.extend(list(to_numpy(indices, args.cuda)))
            zs.append(to_numpy(output['z'], args.cuda))
            atts.append(to_numpy(output['att'], args.cuda))
            if args.model == 'act':
    ...
    if args.model == 'act' and not return_acc:
...
def embed(args, model, loader):
    ...
    with torch.no_grad():
        for batch_idx, (data, target, indices) in enumerate(loader):
            ...
            zs.append(to_numpy(output['z'], args.cuda))
            labels.extend(list(to_numpy(target, args.cuda)))
            all_indices.extend(list(to_numpy(indices, args.cuda)))
            att_matrix = to_numpy(output['att'], args.cuda)
