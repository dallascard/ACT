...
class BaselineModel(nn.Module):
    def __init__(self, args, vocab, embeddings_matrix):
        ...
        if self.base_model == 'cnn':
            ...
            self.lstm_layer = nn.LSTM(input_size=self.embedding_dim, hidden_size=self.hidden_dim//2, bidirectional=True)
        else:
            raise RuntimeError("Base model {:s} not recognized".format(self.base_model))
